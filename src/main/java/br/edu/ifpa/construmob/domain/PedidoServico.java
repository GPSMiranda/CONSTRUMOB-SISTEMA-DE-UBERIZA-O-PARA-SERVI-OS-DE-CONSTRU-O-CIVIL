...
public class PedidoServico {
 ...
 @OneToOne(cascade=CascadeType.ALL,orphanRemoval=true) @JoinColumn(name="pagamento_id") private Pagamento pagamento;
 ...
 public void definirPagamento(Pagamento pagamento){if(pagamento==null||!pagamento.processarValidacao())throw new IllegalArgumentException("Dados de pagamento inválidos.");this.pagamento=pagamento;}
 public void finalizar(){if(status!=StatusPedido.ACEITO)throw new IllegalStateException("O pedido deve estar aceito antes da finalização.");if(pagamento==null||!pagamento.isValidado())throw new IllegalStateException("Defina um pagamento válido antes de finalizar.");this.status=StatusPedido.FINALIZADO;}
