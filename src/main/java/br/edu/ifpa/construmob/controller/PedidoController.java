...
@Controller @RequestMapping("/pedidos") public class PedidoController{
 ...
 @GetMapping("/{numero}") public String detalhe(@PathVariable Long numero,Model model){model.addAttribute("pedido",pedidoService.buscar(numero));model.addAttribute("profissionais",profissionalService.listar());return "pedidos/detalhe";}
 ...
 @PostMapping("/{numero}/finalizar") public String finalizar(@PathVariable Long numero,RedirectAttributes flash){return executar(()->pedidoService.finalizar(numero),numero,"Serviço finalizado.",flash);}
